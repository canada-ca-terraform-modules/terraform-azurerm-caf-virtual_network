...
| `variables.tf` | `env`, `location` (defaults to `canadacentral`), `tags`, `pr_number` (defaults to `"manual"`), and `virtual_network` (typed `any`, passed straight through to the module). |
| `config/virtual_network.tfvars` | One representative real-usage fixture: a single VNet in the throwaway live-test resource group, one address space, no `for_each` fan-out. |
...
cd test/live
terraform init
terraform plan  -var-file=config/virtual_network.tfvars
terraform apply -var-file=config/virtual_network.tfvars
```

Confirm only the live-test resource group and `module.virtual_network` are
planned/applied, then tear it down:

```bash
terraform destroy -var-file=config/virtual_network.tfvars
```
...
terraform apply -var-file=config/virtual_network.tfvars -var="pr_number=<pr-number>"

# 2. PR plan (and, in CI, apply), from A, against the same state file.
cd A/test/live
terraform init -backend-config="path=$STATE"
terraform plan -var-file=config/virtual_network.tfvars -var="pr_number=<pr-number>"

# 3. Always tear down from A once the run finishes (`if: always()` in CI).
terraform destroy -var-file=config/virtual_network.tfvars -var="pr_number=<pr-number>"
```
